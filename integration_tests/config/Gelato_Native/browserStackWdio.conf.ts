...
const config = {
    ...
    afterStep(uri, feature, scenario) {
        ...
        if (!scenario.passed) {
            const reason = scenario.error;
            log.debug(`reason is: ${reason}`);
            driver.takeScreenshot();
            driver.execute(`browserstack_executor: {"action": "setSessionStatus", "arguments": {"status":"failed","reason": "Test is Failed, Reason: ${getFirstLine(reason)}"}}`);
        } else {
            driver.execute(`browserstack_executor: {"action": "setSessionStatus", "arguments": {"status":"passed","reason": "Test is Passed"}}`);
        }
    },
